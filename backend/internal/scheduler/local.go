...
type Local struct{}
func(Local)Submit(ctx context.Context,j domain.TrainingJob)error{for _,e:=range []string{"queue","allocate","start","succeed"}{s,err:=domain.Transition(j.State,e);if err!=nil{return err};j.State=s;if _,err=repository.UpdateJobState(j.ID,s);err!=nil{return err}};return nil}
