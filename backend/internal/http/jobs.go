...
func createJob(c *gin.Context){var j domain.TrainingJob;if c.ShouldBindJSON(&j)!=nil{Error(c,400,"BAD_INPUT","invalid training job",nil);return};j.ID=stringID();j.ProjectID=c.Param("projectId");created,err:=repository.CreateJob(j,c.GetHeader("Idempotency-Key"));if err!=nil{Error(c,400,"BAD_INPUT",err.Error(),nil);return};if err=localScheduler.Submit(c,created);err!=nil{Error(c,500,"SCHEDULER_ERROR",err.Error(),nil);return};c.JSON(http.StatusCreated,created)}
func cancelJob(c *gin.Context){j,ok:=repository.GetJob(c.Param("id"));if !ok||j.ProjectID!=c.Param("projectId"){Error(c,404,"JOB_NOT_FOUND","job not found",nil);return};if err:=localScheduler.Cancel(c,j.ID);err!=nil{Error(c,409,"INVALID_STATE",err.Error(),nil);return};j,_=repository.GetJob(j.ID);c.JSON(200,j)}
